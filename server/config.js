export const config = {
  ...
  queryTimeoutMs: (() => {
    const raw = parseInt(process.env.QUERY_TIMEOUT_MS || String(10 * 60 * 1000), 10);
    const ms = Number.isFinite(raw) ? raw : 10 * 60 * 1000;
    return Math.min(Math.max(ms, 5000), 30 * 60 * 1000); // 5s min, 30min max
  })(),
