...
- **Documentation**: This repository’s docs do not contain real secrets, API keys, or credentials; only placeholders and setup instructions
...
- **Excel export**: Uses `write-excel-file` (no known critical vulnerabilities). Avoid adding packages that have known Prototype Pollution or ReDoS issues.
...
- ✅ Query timeout (10 minutes default)
...
- ✅ Read-only SQL enforcement (DDL/DML blocked); query timeout and row limit mitigate abuse
